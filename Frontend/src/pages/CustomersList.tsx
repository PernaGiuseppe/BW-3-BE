        ...
        <Col>
          <Table hover responsive>
            ...
            <tbody>
              {filtered.map((c) => (
                <tr key={c.id}>
                  <td>{c.logoAziendale ? <img src={c.logoAziendale} alt="logo" style={{ width: 40, height: 40 }} /> : '-'}</td>
                  <td>{c.ragioneSociale}</td>
                  <td>{c.tipo}</td>
                  <td>{c.email}</td>
                  <td>€ {c.fatturatoAnnuale.toLocaleString('it-IT')}</td>
                  <td>{new Date(c.dataUltimoContatto).toLocaleDateString('it-IT')}</td>
                  <td>
                    <Button as={Link} to={`/customers/${c.id}`} size="sm">Dettagli</Button>
                  </td>
                </tr>
              ))}
